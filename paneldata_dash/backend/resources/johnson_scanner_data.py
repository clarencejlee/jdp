...
from libs.csv_parser_helper import parse_data_from_csv, parse_metadata_from_csv, get_type_by_name
...
SHOW_TABLES_QUERY = "SELECT * FROM pg_catalog.pg_tables where schemaname = 'public' and tablename != 'users';"
DESCRIBE_TABLES_QUERY = "SELECT * FROM information_schema.COLUMNS WHERE TABLE_NAME = '{0}'"
...
def get_describe_query(table):
    return DESCRIBE_TABLES_QUERY.format(table)
...
class JohnsonScannerDataCustomQuery(Resource):
    ...
    def get(cls, query: str):
        ...
        if "select" not in filtered_query and "show tables" not in filtered_query\
                and "describe" not in filtered_query:
            return {"error": "Not a select statement"}

        if 'show tables' in filtered_query:
            filtered_query = SHOW_TABLES_QUERY

        if 'describe' in filtered_query:
            filtered_query = get_describe_query(extract_table_name(filtered_query))

        try:
            result = db.session.execute(filtered_query)
            # decide based on the table which schema

            return sql_list_schema.jsonify(result)
    ...
    def post(cls):
        ...
        try:
            ...
            for item in category_data:
                ...
                market = MarketModel.find_by_name(get_type_by_name(data, item, "market"))
                if market:
                    cat.market_id = market.id
                cat.save_to_db()

            for item in brand_data:
                brand = BrandModel(name=item)
                category = CategoryModel.find_by_name(get_type_by_name(data, item, "category"))
                if category:
                    brand.categories_id = category.id
                brand.save_to_db()
